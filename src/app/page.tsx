...
export default function Home() {
  ...
  async function handleLogin() {
    ...
    if (role === 'student') {
      ...
      if (VALID_IDS.includes(upperInput)) {
        setStudentId(upperInput)
        sessionStorage.setItem('kadare_student_id', upperInput)
        setPwInput('')
        return
      }
    }
    ...
    // 4. 班別（グループパスワード）
    const grp = Object.entries(GROUP_PASSWORDS).find(([, pw]) => pw === input)?.[0]
    if (grp) {
      setRole('group')
      setGroupView(grp)
  ...
  function removePhoto(i: number) {
    URL.revokeObjectURL(photoPreviews[i]) // メモリ解放
  ...
  async function handleSubmit() {
    ...
    try {
      for(let i = 0; i < photoFiles.length; i++) {
        ...
        // 2枚目以降も名前と班は保持するように変更（誰の投稿か分かるように）
        const { data: inserted, error } = await supabase.from('posts').insert({
          group_name: group, 
          comment: i === 0 ? comment : `(画像 ${i+1}/${photoFiles.length})`,
          photo_url: photoData.publicUrl,
          audio_url: i === 0 ? audioPublicUrl : null,
          student_name: studentName,
          student_id: studentId // DBにカラムがある場合
        }).select().single()
  ...
  async function handleLike(post: Post) {
    ...
    try {
      ...
      const { error } = await supabase.from('posts').update({ like_count: newCount }).eq('id', post.id)
      if (error) throw error
  ...
  if(!role) return (
    <div style={{minHeight:'100vh',background:'#f5f0e8',display:'flex',justifyContent:'center',alignItems:'center',fontFamily:'Georgia,serif'}}>
      <div style={{background:'#fff',padding:'40px 32px',borderRadius:8,width:'100%',maxWidth:360,boxShadow:'0 2px 12px rgba(0,0,0,0.1)'}}>
        ...
        {pwError && <p style={{color:'#c0392b',fontSize:12,marginBottom:8}}>{pwError}</p>}
  ...
  if (role === 'student' && !studentId) return (
    <div style={{ minHeight:'100vh', background:'#f5f0e8', display:'flex', alignItems:'center', justifyContent:'center', padding:24 }}>
      <div style={{ background:'#fff', padding:'40px 32px', borderRadius:20, width:'100%', maxWidth:420, boxShadow:'0 10px 30px rgba(0,0,0,.08)' }}>
        <div style={{ fontSize:22, fontWeight:700, marginBottom:12, textAlign:'center' }}>学籍番号を入力してください</div>
        <label style={{ display:'block', fontSize:12, fontWeight:700, marginBottom:8 }}>学籍番号</label>
        <input value={pwInput} onChange={(e)=>setPwInput(e.target.value)} onKeyDown={e=>e.key==='Enter'&&handleLogin()} placeholder="（例）B28C001" style={{ width:'100%', padding:'14px 16px', border:'1px solid #ddd', borderRadius:12, fontSize:16, marginBottom:16 }} />
        {pwError && <p style={{ color:'#c0392b', fontSize:14, marginBottom:12 }}>{pwError}</p>}
        <button onClick={handleLogin} style={{ width:'100%', padding:'14px 16px', border:'none', borderRadius:12, background:'#333', color:'#fff', fontSize:16, fontWeight:700, cursor:'pointer' }}>進む</button>
      </div>
    </div>
  )
  ...
  return (
    <div style={{minHeight:'100vh',background:'#f5f0e8',fontFamily:'Georgia,serif',color:'#1a1a1a'}}>
      <header style={{background:'#1a1a1a',position:'sticky',top:0,zIndex:100}}>
        <div style={{maxWidth:680,margin:'0 auto',padding:'12px 16px',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
          ...
          <nav style={{display:'flex',gap:8,alignItems:'center'}}>
            ...
            <button onClick={()=>setScreen('gallery')} style={{background:screen==='gallery'?'#c9963a':'none',border:'1px solid #444',color:'#ccc',padding:'6px 14px',borderRadius:20,cursor:'pointer',fontSize:13}}>一覧</button>
            <button onClick={handleLogout} style={{background:'none',border:'1px solid #666',color:'#ccc',padding:'6px 14px',borderRadius:20,cursor:'pointer',fontSize:13}}>ログアウト</button>
          </nav>
        </div>
      </header>
          ...
          <div>
            ...
            {filtered.length===0 ? <p style={{textAlign:'center',color:'#999',marginTop:40}}>投稿がありません</p> : (
              <div style={{display:'grid',gridTemplateColumns:'repeat(2,1fr)',gap:12}}>
                {filtered.map(p=>(
                  <div key={p.id} onClick={()=>{setSelected(p);setEditing(false);setEditComment(p.comment||'');setEditGroup(p.group_name)}} style={{background:'#fff',borderRadius:4,overflow:'hidden',cursor:'pointer',boxShadow:'0 1px 4px rgba(0,0,0,0.1)',display:'flex',flexDirection:'column'}}>
                    ...
                    <div style={{padding:'10px 12px', flex:1}}>
                      ...
                      {p.comment && <p style={{fontSize:12,color:'#444',lineHeight:1.5,marginBottom:4, overflow:'hidden', textOverflow:'ellipsis', display:'-webkit-box', WebkitLineClamp:2, WebkitBoxOrient:'vertical'}}>{p.comment}</p>}
                    ...
                    <div style={{padding:'0 12px 12px', display:'flex', justifyContent:'space-between', alignItems:'center'}}>
                      <button onClick={(e) => { e.stopPropagation(); handleLike(p) }} disabled={likedPostIds.includes(p.id)} style={{ background: likedPostIds.includes(p.id) ? '#eee' : '#fff', color: likedPostIds.includes(p.id) ? '#999' : '#1a1a1a', border: '1px solid #ccc', padding: '4px 8px', borderRadius: 20, cursor: likedPostIds.includes(p.id) ? 'default' : 'pointer', fontSize: 11 }}>
                        {likedPostIds.includes(p.id) ? '♥' : '♡'} {p.like_count || 0}
                      </button>
                    </div>
                  </div>
                ))}
      ...
      {/* 詳細・編集モーダル */}
      {selected && (
        <div onClick={()=>{setSelected(null);setEditing(false)}} style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.8)',zIndex:200,display:'flex',alignItems:'flex-end',justifyContent:'center'}}>
          <div onClick={e=>e.stopPropagation()} style={{background:'#fff',width:'100%',maxWidth:600,maxHeight:'95vh',overflowY:'auto',borderRadius:'16px 16px 0 0',position:'relative'}}>
            <button onClick={()=>{setSelected(null);setEditing(false)}} style={{position:'absolute',top:12,right:12,background:'rgba(255,255,255,0.8)',border:'none',width:36,height:36,borderRadius:'50%',cursor:'pointer',fontSize:18,zIndex:1,fontWeight:700}}>✕</button>
            <img src={selected.photo_url} style={{width:'100%',maxHeight:'60vh',objectFit:'contain',background:'#000'}} alt="" />
            <div style={{padding:20}}>
              {editing ? (
                <div>
                  ...
                  <textarea value={editComment} onChange={e=>setEditComment(e.target.value)} rows={4} style={{width:'100%',border:'2px solid #ccc',borderRadius:4,padding:'10px',fontSize:14,resize:'vertical',boxSizing:'border-box',marginBottom:12}} />
                  <div style={{display:'flex',gap:8}}>
                    <button onClick={()=>handleEdit(selected)} style={{flex:1,background:'#1a1a1a',color:'#fff',border:'none',padding:'12px',borderRadius:4,cursor:'pointer',fontSize:14,fontWeight:700}}>保存する</button>
                    <button onClick={()=>setEditing(false)} style={{flex:1,background:'#eee',color:'#333',border:'none',padding:'12px',borderRadius:4,cursor:'pointer',fontSize:14}}>キャンセル</button>
                  </div>
                </div>
              ) : (
                <div>
                  <div style={{display:'flex',gap:8,marginBottom:12,alignItems:'center'}}>
                    <span style={{background:'#1a1a1a',color:'#fff',fontSize:12,padding:'4px 10px',borderRadius:4,fontWeight:700}}>{selected.group_name}班</span>
                    {selected.student_name && <span style={{background:'#eee',color:'#555',fontSize:12,padding:'4px 10px',borderRadius:4}}>{selected.student_name}</span>}
                    <span style={{flex:1}} />
                    <span style={{fontSize:12,color:'#999'}}>{new Date(selected.created_at).toLocaleString('ja-JP')}</span>
                  </div>
                  {selected.audio_url && <audio src={selected.audio_url} controls style={{width:'100%',marginBottom:16}} />}
                  {selected.comment && <p style={{fontSize:16,lineHeight:1.8,color:'#1a1a1a',marginBottom:20, whiteSpace:'pre-wrap'}}>{selected.comment}</p>}
                  
                  <div style={{display:'flex',gap:8,borderTop:'1px solid #eee',paddingTop:16}}>
                    <button onClick={() => handleLike(selected)} disabled={likedPostIds.includes(selected.id)} style={{ flex:1, background: likedPostIds.includes(selected.id) ? '#eee' : '#fff', color: likedPostIds.includes(selected.id) ? '#999' : '#1a1a1a', border: '1px solid #ccc', padding: '12px', borderRadius: 8, cursor: likedPostIds.includes(selected.id) ? 'default' : 'pointer', fontSize: 14, fontWeight:700 }}>
                      {likedPostIds.includes(selected.id) ? '♥ いいね済み' : '♡ いいね'} ({selected.like_count || 0})
                    </button>
                    
                    {(role==='teacher' || myPostIds.includes(selected.id)) && (
                      <>
                        {myPostIds.includes(selected.id) && (
                          <button onClick={()=>setEditing(true)} style={{flex:1,background:'#4a87b8',color:'#fff',border:'none',padding:'12px',borderRadius:8,cursor:'pointer',fontSize:14}}>✏️ 編集</button>
                        )}
                        <button onClick={()=>handleDelete(selected)} style={{flex:1,background:'#c0392b',color:'#fff',border:'none',padding:'12px',borderRadius:8,cursor:'pointer',fontSize:14}}>🗑 削除</button>
                      </>
                    )}
                  </div>
                </div>
