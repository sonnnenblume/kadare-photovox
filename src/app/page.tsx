  ...
  async function loadPosts() {
    try {
      ...
      if (role === 'teacher') {
        ...
        // グループ(GroupA-H)は、自分の班の名前(group_name)と一致する分だけ
        query = query.eq('group_name', userId)
      } else if (role === 'student') {
        // 学生(学籍番号)は、自分のID(user_id)と一致する分だけ
      ...
      <main style={{ maxWidth: '600px', margin: '0 auto', padding: '20px' }}>
        {!role ? (
          ...
          <div style={{ paddingTop: '40px', display: 'flex', flexDirection:'column', gap: '20px' }}>
            ...
            {role === 'viewer' && <p style={{ textAlign: 'center', color: '#666' }}>※{userId}の投稿のみ表示されます</p>}
          </div>
        ) : screen === 'upload' ? (
          ...
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              <h2>{role === 'teacher' ? '全投稿一覧' : 'あなたの投稿'}</h2>
